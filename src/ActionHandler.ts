...
export class ActionHandler {
	...
	private extractExtraFields(action: CompanionActionEvent, data_type: string) {
		return Object.keys(action.options)
			.reduce((filtered, key) => {
	...
	handleToggle(action: CompanionActionEvent): void {
		...
		this.log('info', '➡️ sending toggle ' + JSON.stringify({
			'group_addr': group_addr,
			'data_type': data_type + '.' + data_subtype,
	...
	private evaluateFeedback(action: CompanionActionEvent): boolean {
		...
		const feedback_fields = Object.keys(action.options)
			.reduce((filtered, key) => {
		...
		this.log('info', '⬅ check feedback for action ' + JSON.stringify({
			'group_addr': group_addr,
			'data_type': data_type + '.' + data_subtype,
			'raw_value': raw_value,
