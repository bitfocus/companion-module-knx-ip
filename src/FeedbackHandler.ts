...
} from '@companion-module/base/dist/module-api/feedback'
...
export class FeedbackHandler {
	...
	private handle(feedback: CompanionFeedbackBooleanEvent): boolean {
		...
		const extra_fields = Object.keys(feedback.options).reduce(
			(filtered, key) => {
		...
		const feedback_fields = Object.keys(feedback.options).reduce(
			(filtered, key) => {
